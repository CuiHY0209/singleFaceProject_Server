...
namespace parser
{
	inline int parse(const std::string &input, cv::Mat &rgb, cv::Mat &depth, long long &ts)
	{
		try
		{
			...
			ts = received["time"].get<long long>();
	...
	inline int parseCommaSplit(const std::string &input, cv::Mat &rgb, cv::Mat &depth, long long &ts)
	{
		try
		{
