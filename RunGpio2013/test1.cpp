...
//	long ts;
//	parser::parse(input, inputRGB, inputDepth, ts);
//
//	cout << "行：" << inputRGB.rows << "列:" << inputRGB.cols << "channeks" << inputRGB.channels() << endl;
//	cout << "inputDepth行：" << inputDepth.rows << "inputDepth列:" << inputDepth.cols << "inputDepthchanneks" << inputDepth.channels() << endl;
//	//----------------------------------------------------------------------
//	
//	/*cv::imshow("test", inputRGB);
//	cv::waitKey(0);*/
//	cv::imshow("test2", inputDepth);
//	cv::waitKey(0);
//	timeFlag[0] = clock();
//
//	RGBD2EIA rgbd2eia;
//	//cv::Mat inputRGB = cv::imread("res/video_depth/0.jpg", cv::IMREAD_COLOR);
//	//cv::Mat inputDepth = cv::imread("res/video_depth/0_depth.jpg", cv::IMREAD_GRAYSCALE);
//	
//	/*开辟接收数据用的内存空间*/
